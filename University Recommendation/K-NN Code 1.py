...
y = data["University"]  # Replace "target_variable" with the column name of the target variable

# Step 3: Splitting the Dataset
X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.6, random_state=577)
...
# Implement an interface to take user preferences and generate personalized recommendations based on the trained model
...
def generate_recommendations(preferences, knn_model):
    ...
    # Scale the user preferences using the same scaler used on the training data
    user_preferences_scaled = scaler.transform(user_preferences_df)
...
recommended_universities = generate_recommendations(user_preferences, knn)
